...
***We do this because often because the median doesn't take lower or upper extremes into account like the mean does, which will give us a more accurate picture of the data using the median.***
...
I choose a binwidth of 5000 because it gives the viewer enough detail without being overly complex to interpret. Also, using a binwidth of 1000 caused the median income to read with scientific notation which is even harder to read.  
...
4.  Based on the shape of the histogram you created in the previous exercise, determine which of these summary statistics is useful for describing the distribution.
   ...
   ***The distribution is positively skewed with a sharp incline from the start and then proceeding into a less but still sharp decline over the back half. The center of the data is the median right at 36,000. The spread goes from 22,000 to 110,000, but most of distribution is less than 60,000. The most unusual observation is the median job income at 110,000 because it is isolated from the rest of the data. I think the best summary statistic to use is the median because it describes the experience of the majority of college graduates. The output of the median is 36,000.***
...
```{r fig2, height=2}
ggplot(data = college_recent_grads, mapping = aes(x = median, fill = major_category)) +
  geom_histogram(binwidth = 5000)
...
```{r fig3, height=2}
ggplot(data = college_recent_grads, mapping = aes(x = median, y = sharewomen, colour = major_type)) +
geom_point()
```
***According to the plot, it appears that a higher concentration of women major in non-stem fields, and that the stem women have a higher median income on average than the non-stem women***
...
```{r fig4, height=2}
ggplot(data = college_recent_grads, mapping = aes(x = unemployment_rate, fill = major_type)) +
  geom_histogram(bindwidth = 30)
 
```
***How does the unemployment rate of stem majors compare to non-stem majors? According to the visualization, it is apparent that the unemployment rate for stem majors is lower on average compared to non-stem majors.***
